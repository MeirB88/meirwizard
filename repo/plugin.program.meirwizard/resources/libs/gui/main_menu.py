...
class MainMenu:
    ...
    def get_listing(self):
        ...
        errorsfound = str(errors) + 'שגיאות נמצאו' if errors > 0 else 'אין שגיאות'
        ...
        if len(CONFIG.BUILDNAME) > 0:
            version = check.check_build(CONFIG.BUILDNAME, 'גרסה')
            ...
            if themefile:
                directory.add_file('לא מותקן' if CONFIG.BUILDTHEME == "" else CONFIG.BUILDTHEME, {'mode': 'theme', 'name': CONFIG.BUILDNAME},
                                   themeit=CONFIG.THEME5)
        else:
            directory.add_dir('לא מותקן', {'mode': 'builds'}, themeit=CONFIG.THEME4)
        ...
        if (tools.platform() == 'android' or CONFIG.DEVELOPER == 'true'):
            directory.add_dir('התקנת אפליקציה', {'mode': 'apk'}, icon=CONFIG.ICONAPK, themeit=CONFIG.THEME1)
        ...
        directory.add_dir('שמירת נתונים', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
        ...
        directory.add_file('הצג שגיאות בלוג: {0}'.format(errorsfound), {'mode': 'viewerrorlog'}, icon=CONFIG.ICONMAINT,
                           themeit=CONFIG.THEME1)
        if errors > 0:
            directory.add_file('הצג שגיאה אחרונה בלוג', {'mode': 'viewerrorlast'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME1)
        directory.add_separator()
