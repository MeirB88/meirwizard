...
def install_from_kodi(plugin):
    ...
    if xbmc.getCondVisibility(installed_cond):
        logging.log('הותקן כבר ' + plugin, level=xbmc.LOGDEBUG)
        return True
    ...
    while not xbmc.getCondVisibility(installed_cond):
        if time.time() >= start + timeout:
            logging.log('נגמר זמן ההתקנה', level=xbmc.LOGDEBUG)
            return False
        ...
        if xbmc.getCondVisibility(visible_cond) and not clicked:
            logging.log('לחיצה לפתיחה', level=xbmc.LOGDEBUG)
            xbmc.executebuiltin('SendClick(yesnodialog, 11)')
            clicked = True
        else:
            logging.log('ממתין...', level=xbmc.LOGDEBUG)

    logging.log('הותקן {0}!'.format(plugin), level=xbmc.LOGDEBUG)
    return True
...
class AddonMenu:
    ...
    def show_menu(self, url=None):
        ...
        if url_response:
            ...
            TEMPADDONFILE = None
            logging.log("[תפריט הרחבות] לא נוספה רשימת הרחבות")
        ...
        if TEMPADDONFILE:
            ...
            try:
                ...
                addons_json = None
                logging.log("[Advanced Settings] שגיאה: פורמט שגוי עבור {0}.".format(TEMPADDONFILE))
            ...
            if addons_json:
                ...
                if addons and len(addons) > 0:
                    for addon in addons:
                        ...
                        if not addonname:
                            logging.log('[Advanced Settings] תיוג חסר \'name\'', level=xbmc.LOGDEBUG)
                            continue

                        if not addonurl:
                            logging.log('[Advanced Settings] תיוג חסר \'url\'', level=xbmc.LOGDEBUG)
                            continue
                        else:
                            if '.zip' in addonurl:
                                pass
                                ...
                                broken = False
                                if not repository:
                                    logging.log('[Advanced Settings] תיוג חסר \'repository\'', level=xbmc.LOGDEBUG)
                                    broken = True
                                if not repositoryxml:
                                    logging.log('[Advanced Settings] תיוג חסר \'repositoryxml\'',
                                                level=xbmc.LOGDEBUG)
                                    broken = True
                                if not repositoryurl:
                                    logging.log('[Advanced Settings] תיוג חסר \'repositoryurl\'',
                                                level=xbmc.LOGDEBUG)
                                    broken = True
                                if broken:
                                    continue
                    ...
                    if not addons:
                        ...
                        logging.log("[תפריט הרחבות] שגיאה: פורמט שגוי")
                    elif len(addons) == 0:
                        directory.add_file("לא נוספו הרחבות לתפריט זה!", themeit=CONFIG.THEME2)
        else:
            logging.log("[תפריט הרחבות] שגיאה: לינק לרשימת הרחבות שגוי")
    ...
    def install_addon_from_url(self, plugin, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]התקנת הרחבות[/COLOR]".format(CONFIG.COLOR1),
                               '[COLOR {0}]{1}:[/COLOR] [COLOR {2}]קישור שגוי[/COLOR]'.format(CONFIG.COLOR1,
                                                                                                    plugin,
                                                                                                    CONFIG.COLOR2))
            return
        ...
        self.progress_dialog.update(0, title
                                    +'\n'+''
                                    +'\n'+'[COLOR {0}]מתקין תלויות[/COLOR]'.format(CONFIG.COLOR2))
        installed(plugin)
    ...
    def install_addon(self, plugin, urls, over=False):
        ...
        if not over:        
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(plugin)):
                install = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]{1}[/COLOR] מותקן כבר. תרצו להתקין שוב?'.format(CONFIG.COLOR1, plugin))
            else:
                install = self.dialog.yesno(CONFIG.ADDONTITLE, 'האם תרצו להתקין את [COLOR {0}]{1}[/COLOR]?'.format(CONFIG.COLOR1, plugin))
        else:
            install = True
        ...
        if False not in [repositoryxml_response, repositoryurl_response]:
            ...
            if not xbmc.getCondVisibility('System.HasAddon({0})'.format(repo_id)):
                logging.log("מקור לא מותקן, מתקין עכשיו.")
                ...
                if version:
                    ...
                    logging.log(
                        "[מתקין הרחבות] התקנה נכשלה, לא ניתן להשיג קישור ({0})".format(urls[1]))
            else:
                logging.log("מקור הותקן, מתקין הרחבה")
                install = install_from_kodi(plugin)
                if install:
                    xbmc.executebuiltin('Container.Refresh()')
                    return True
        elif url_response:
            logging.log("אין מקור, מתקין הרחבה")
    ...
    def install_addon_pack(self, name, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]מתקין הרחבות[/COLOR]".format(CONFIG.COLOR1),
                               '[COLOR {0}]{1}:[/COLOR] [COLOR {2}]קישור לקובץ זיפ שגוי![/COLOR]'.format(CONFIG.COLOR1, name, CONFIG.COLOR2))
            return
        ...
        logging.log_notify("[COLOR {0}]מתקיו הרחבות[/COLOR]".format(CONFIG.COLOR1),
