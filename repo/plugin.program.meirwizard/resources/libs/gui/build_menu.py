...
class BuildMenu:
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('קישור לקובץ טקסט שגוי', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('גירסת קודי: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('תפריט שמירת נתונים', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
        directory.add_separator()
        ...
        if len(match) >= 1:
            if CONFIG.SEPARATE == 'true':
                ...
                if count19 > 0:
                    ...
                    directory.add_file('[B]{0} בילדים לקודי 19 ({1})[/B]'.format(state, count19), {'mode': 'togglesetting',
            ...
            if adultcount > 0:
                directory.add_file('בילדים למבוגרים בלבד', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
                directory.add_file('הפעל בילדים למבוגרים בלבד בהגדרות', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
            else:
                directory.add_file('אין בילדים {0}'.format(CONFIG.ADDONTITLE),
                                   icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        else:
            directory.add_file('קובץ בילדים בפורמט שגוי', icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
    ...
    def view_build(self, name):
        ...
        if response:
            ...
            directory.add_file('קישור לקובץ טקסט שגוי', themeit=CONFIG.THEME3)
            directory.add_file('{0}'.format(CONFIG.BUILDFILE), themeit=CONFIG.THEME3)
            return

        if not check.check_build(name, 'version'):
            directory.add_file('שגיאה בקריאת קובץ טקסט', themeit=CONFIG.THEME3)
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            if updatecheck:
                build = '{0} [COLOR red][נוכחי v{1}][/COLOR]'.format(build, CONFIG.BUILDVERSION)
            ...
            if previewcheck:
                directory.add_file('הצג סרטון הסבר', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME3)
            
            if versioncheck:
                directory.add_file(
                    '[I]הבילד נבנה עבור קודי v{0} (מותקן: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
                    fanart=fanart, icon=icon, themeit=CONFIG.THEME3)
                    
            directory.add_separator('התקנה')
            ...
            if guicheck:
                directory.add_file('עדכון מהיר', {'mode': 'install', 'action': 'gui', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
            ...
            if themecheck:
                ...
                for themename, themeurl, themeicon, themefanart, themeadult, description in match:
                    ...
                    themetitle = themename if not themename == CONFIG.BUILDTHEME else "[B]{0} (מותקן)[/B]".format(themename)
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg += "[COLOR {0}]גירסה:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, version)
                if themes:
                    msg += "[COLOR {0}]ערכת נושא של הבילד:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, ', '.join(themes))
                msg += "[COLOR {0}]גירסת קודי:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, kodi)
                ...
                if extend:
                    ...
                    msg += "[COLOR {0}]גודל זיפ:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(zipsize))))
                    ...
                    msg += "[COLOR {0}]בינאריים:[/COLOR] [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, binaries)

                window.show_text_box("מציג פרטי בילד: {0}".format(name), msg)
            else:
                logging.log("שם הבילד שגוי!")
        else:
            logging.log("קובץ הבילדים לא עובד {0}".format(CONFIG.BUILDFILE))
    ...
    def build_video(self, name):
        ...
        if response:
            videofile = check.check_build(name, 'תצוגה מוקדמת:')
            if tools.open_url(videofile, check=True):
                yt.play_video(videofile)
            else:
                logging.log("[{0}]אין קישור".format(name))
        else:
            logging.log("קובץ הבילדים לא עובד: {0}".format(CONFIG.BUILDFILE))
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('עדכון מהיר'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=gui)'.format(CONFIG.ADDON_ID, buildname)))
