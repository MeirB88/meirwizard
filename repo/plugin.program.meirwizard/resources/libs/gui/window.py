...
def show_apk_warning(apk):
    ...
    xbmc.executebuiltin('Skin.SetString(apkinstaller, עכשיו כש {0} ירד בהצלחה, [CR]לחץ התקן בחלון הבא!)'.format(apk))
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def do_build_menu(self):
            logging.log("[בדיקת בילד נוכחי] [המשתמש בחר: תפריט בילדים פתוח] [בדיקה: {0}]".format(CONFIG.BUILDCHECK),
                        level=xbmc.LOGINFO)
        ...
        def do_ignore(self):
            logging.log("[בילד נוכחי] [בחירת המשתמש: התעלמות] [בדיקה הבאה: {0}]".format(CONFIG.BUILDCHECK),
                        level=xbmc.LOGINFO)
            CONFIG.set_setting('nextbuildcheck', tools.get_date(days=CONFIG.UPDATECHECK, formatted=True))
            CONFIG.set_setting('installed', 'ignored')
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            self.msgupdate = "קיים עדכון לבילד מותקן[COLOR {0}]{1}[/COLOR]\n\nגרסה נוכחית: v[COLOR {2}]{3}[/COLOR]\nגרסה אחרונה: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*מומלץ לבצע התקנה נקייה[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgcurrent = "הבילד מעודכן:\n[COLOR {0}]{1}[/COLOR]\n\nגרסה נוכחית: v[COLOR {2}]{3}[/COLOR]\nגרסה אחרונה: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*מומלץ לבצע התקנה נקייהl[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
        ...
        def do_fresh_install(self):
            logging.log("[בדוק עדכונים] [גרסה מותקנת: {0}] [גרסה נוכחית: {1}] [סוג התקנה: נקייה]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log("[בדיקת עדכונים] [בדיקה הבאה: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
        ...
        def do_normal_install(self):
            logging.log("[בדוק עדכונים] [גרסה מותקנת: {0}] [גרסה נוכחית: {1}] [סוג התקנה: רגילה]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log("[בדיקת עדכונים] [הבדיקה הבאה: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            url = 'plugin://{0}/?mode=install&name={1}&action=normal'.format(CONFIG.ADDON_ID, quote_plus(CONFIG.BUILDNAME))
            xbmc.executebuiltin('RunPlugin({0})'.format(url))
            self.close()

        def do_ignore(self):
            logging.log("[בדוק עדכונים] [גרסה מותקנת: {0}] [גרסה נוכחית: {1}] [בחירה: התעלם {2} ימים]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST, CONFIG.UPDATECHECK))
            logging.log("[בדוק עדכונים] [בדיקה הבאה: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            self.close()
    ...
    # update.doModal()
    # del update
    msgcurrent = 'הבילד מעודכן: '
    msgupdate = 'קיימים עדכונים לבילד: '
    ...
    latest_version = 'גרסה אחרונה: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
...
def show_notification(msg, test=False):
    class Notification(xbmcgui.WindowXMLDialog):
        ...
        def do_remind(self):
            ...
            logging.log('[התראות] התראה {0} הזכר לי מאוחר יותר'.format(CONFIG.get_setting('noteid')))
            self.close()

        def do_dismiss(self):
            if not test:
                CONFIG.set_setting('notedismiss', 'true')
            logging.log('[התראות] התראה {0} משוחרר'.format(CONFIG.get_setting('noteid')))
            self.close()
...
def show_log_viewer(window_title="מציג לוג", window_msg=None, log_file=None, ext_buttons=False):
